// FILE: packages/modules/admin/src/modules/authorization/screens/roles/AccessPolicyBuilder.tsx
// STYLE AUDIT: Aligned to OrganizationProfile.tsx design system
// FIXES: Corrected type collisions by importing AccessPolicy from @cap/shared-types
import React, { useState, useMemo, useEffect, useCallback } from 'react'
import { adminService } from '@cap/module-auth'
import {
  Box,
  Typography,
  Card,
...
import { useSessionGuard } from '@cap/module-auth'
...
export default function AccessPolicyBuilder() {
  ...
  const { data: organizationsResponse, isLoading: loadingOrgs } = useOrganizations({ limit: 100 }) as any
  ...
  const handleDelete = async (policyId: string | number) => {
    if (isInvalidOrgId) {
      enqueueSnackbar(t('auth.admin.invalidOrgId', 'Invalid organization ID'), { variant: 'error' })
      return
    }
                ...
                <FormControl size='small' sx={{ minWidth: 220, ml: 2 }}>
                  ...
                  <Select
                    ...
                    onChange={(e) => {
                      ...
                      if (newId) {
                        navigate(Path.admin.policies.replace(':id', String(newId)))
                      }
                    }}
