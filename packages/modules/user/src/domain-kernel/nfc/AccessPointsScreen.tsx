...
// @ts-ignore
import { Box, Typography, Button, Chip, Avatar, Stack, Card, CardContent, CardActions, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, TextField, FormControl, InputLabel, Select, MenuItem, CircularProgress, Alert, Tooltip, Grid } from '@mui/material'
// @ts-ignore
import { Add, Sensors, Delete, Settings, VpnKey, Wifi, WifiOff, ContentCopy } from '@mui/icons-material'
// @ts-ignore
...
const AccessPointsScreen: React.FC<AccessPointsScreenProps> = ({ orgId }) => {
  ...
  const fetchPoints = useCallback(async () => {
    try {
      setLoading(true)
      const res = await apiClient.get<any>(`/api/admin/organizations/${orgId}/nfc/access-points`)
      if (!res.ok) throw new Error(res.data?.message || 'Failed to load access points')
  ...
  const handleCreate = async () => {
    try {
      setFormLoading(true)
      const res = await apiClient.post<any>(`/api/admin/organizations/${orgId}/nfc/access-points`, formData)
      if (!res.ok) throw new Error(res.data?.message || 'Failed to create')
      const json = res.data
      setRevealDialog({ open: true, token: json.api_token, name: json.name })
      setIsFormOpen(false)
  ...
  const handleRegenerateToken = async (pointId: number, name: string) => {
    ...
    try {
      const res = await apiClient.post<any>(`/api/admin/organizations/${orgId}/nfc/access-points/${pointId}/regenerate-token`)
      if (!res.ok) throw new Error(res.data?.message || 'Regeneration failed')
      const json = res.data.data || res.data
      setRevealDialog({ open: true, token: json.api_token, name })
    } catch (err: unknown) {
  ...
  const handleToggleStatus = async (point: AccessPoint) => {
    try {
      ...
      const res = await apiClient.patch<any>(`/api/admin/organizations/${orgId}/nfc/access-points/${point.id}`, { status: newStatus })
      if (!res.ok) throw new Error(res.data?.message || 'Update failed')
      toast.success(t('nfc.points.status_updated', 'Reader updated'))
      fetchPoints()
    } catch (err: unknown) {
  ...
  const handleDelete = async (pointId: number) => {
    ...
    try {
      const res = await apiClient.delete<any>(`/api/admin/organizations/${orgId}/nfc/access-points/${pointId}`)
      if (!res.ok) throw new Error(res.data?.message || 'Delete failed')
      toast.success(t('nfc.points.deleted', 'Reader deleted'))
      fetchPoints()
    } catch (err: unknown) {
  ...
  const copyToClipboard = (text: string) => {
    ...
    toast.info(t('common.copied', 'Copied to clipboard'), { autoClose: 2000 })
  }
              ...
              <Card variant="outlined" sx={{ borderRadius: 3, transition: '0.2s', '&:hover': { boxShadow: 4, borderColor: 'primary.main' } }}>
                <CardContent>
                  ...
                  <Box bgcolor="action.hover" p={1.5} borderRadius={2} mb={2}>
                    ...
                    <Typography variant="body2" fontFamily="monospace" fontWeight={700}>{point.apiTokenPrefix}••••••••</Typography>
                  </Box>
          ...
          <Stack spacing={2.5} sx={{ mt: 1 }}>
            ...
            <FormControl fullWidth>
              ...
              <Select
                ...
                label="Direction"
                onChange={(e: any) => setFormData({ ...formData, direction: e.target.value })}
              >
