...
const AccessLogsScreen: React.FC<AccessLogsScreenProps> = ({ orgId, apiBase = '/api/admin' }) => {
  ...
  const fetchLogs = useCallback(async () => {
    try {
      ...
      const res = await fetch(`${apiBase}/organizations/${orgId}/nfc/logs?${params}`)
  ...
  }, [apiBase, orgId, page, pageSize, statusFilter, directionFilter])
    ...
    <Box>
      ...
      <Stack direction="row" spacing={2} mb={2} flexWrap="wrap">
        ...
        <FormControl size="small" sx={{ minWidth: 130 }}>
          ...
          <Select
            value={statusFilter}
            onChange={(e) => setStatusFilter(e.target.value as 'all' | 'granted' | 'denied')}
            label="Result"
          >
