...
import { useScrapingProgress, useAnalysisProgress } from '../../hooks/useAuthQuery'
import {
  Box,
  Button,
  Container,
...
const RealTimeAuthEventsMonitor = () => {
  ...
  // Integrate SSE for Demo/Monitoring purpose (targeting a placeholder or current job)
  const { isConnected: isScrapingConnected, data: scrapingData } = useScrapingProgress('current')
  const { isConnected: isAnalysisConnected, data: analysisData } = useAnalysisProgress('active')
  ...
  const stats = [
    ...
    {
      label: 'Events / Sec',
      value: scrapingData?.progress ? `${scrapingData.progress}%` : '85',
      change: '+12%',
      icon: Speed,
      color: theme.palette.primary.main,
    },
