...
import { useCurrentUserQuery, useChangeEmail } from '../../../hooks'
...
function ChangeEmail() {
  ...
  const [successData, setSuccessData] = useState<{ message: string; verifyUrl?: string } | null>(null)
  ...
  const { mutate: changeEmail, isPending } = useChangeEmail({
    ...
    onError: (error) => {
      addNotification({
        ...
        message: error.message || t('auth.account.change_email_error_toast', 'Failed to change email.'),
      })
    },
  })
  ...
  if (successData) {
    return (
      ...
      >
        <Card className='glass-effect' sx={{ maxWidth: 480, width: '100%', borderRadius: 3, textAlign: 'center', p: 4 }}>
          ...
          {successData.verifyUrl && (
            <Alert severity="info" sx={{ mb: 4, textAlign: 'left' }}>
              <Typography variant="body2" fontWeight="bold">Dev Mode: Verification Link</Typography>
              ...
              >
                ...
                <Box>
                  ...
                  <Controller
                    name='password'
                    control={controlForm.control}
                    rules={{ required: t('auth.account.password_required', 'Password is required to confirm identity') }}
                    render={({ field, fieldState }) => (
                      <TextField
                        {...field}
                        id='confirmPassword'
                ...
                >
                  {isPending ? <CircularProgress size={24} color="inherit" /> : t('auth.account.update_email')}
                </Button>
