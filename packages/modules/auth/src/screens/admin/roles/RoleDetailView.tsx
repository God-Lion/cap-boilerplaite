...
export default function RoleDetailView() {
  ...
  const duplicateRoleMutation = useDuplicateRole({
    onSuccess: (response) => {
      ...
      if (newId) navigate(Path.roleDetail.replace(':id', newId.toString()))
    },
  })
    ...
    return (
      <Box sx={{ p: 4, maxWidth: 800, mx: 'auto' }}>
        <Alert
          ...
          action={
            <Button color='inherit' onClick={() => navigate(Path.roles)}>
          ...
          <Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
              <Tooltip title={t('auth.admin.goBack') || 'Go back'}>
                <IconButton onClick={() => navigate(Path.roles)} sx={{ ml: -1 }}>
