...
interface PasskeySetupAutoProps {
  ...
  autoStart?: boolean // Auto-trigger WebAuthn on dialog open
}

/**
 * PasskeySetupAuto - Automatically triggers WebAuthn registration on mount
 *
 * This version immediately shows the "Waiting for confirmation..." state
 * and triggers the browser's biometric prompt when the dialog opens.
 *
 * @example
 * ```tsx
 * <PasskeySetupAuto
 *   open={true}
 *   autoStart={true}  // Triggers immediately
...
export default function PasskeySetupAuto({
  ...
  const handleSetupPasskey = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = err.message || 'Passkey registration cancelled or failed'
  ...
  // Auto-trigger passkey setup when dialog opens
  useEffect(() => {
  ...
  const handleUseSecurityKey = async () => {
    // Same flow but user explicitly chose security key option
  ...
  return (
    ...
    >
      <DialogContent sx={{ px: 4, py: 5, textAlign: 'center' }}>
        ...
        {/* Waiting State - Always visible when autoStart is enabled */}
        ...
        {/* Use Security Key Button - Hidden during auto-start loading */}
