...
The codebase is in noticeably good shape for its size: the documented 6-tier dependency model is enforced at the tooling level (ESLint `import/no-restricted-imports`), a working coupling-analysis script already exists, prior audit passes have fixed real bugs (see `technical-issues.md`), and the theme/design-token system is genuinely sophisticated (15 presets, 7 visual effect generators, tenant-scoped).
...
| `@cap/theme` | `packages/theme` | Design tokens, `composeMuiTheme`, tenant theme context, 15 presets, 7 visual-effect generators (glass/neu/bento/brutalism/organic/immersive), MUI component overrides. |
...
**Verified against `pnpm-workspace.yaml`:** the workspace glob is exactly `app`, `packages/auth-contracts`, `packages/api-contracts`, `packages/layout`, `packages/platform-core`, `packages/platform-store`, `packages/shared-types`, `packages/theme`, `packages/modules/**`. Only `auth` and `landing` exist under `packages/modules/`.

---

## 3. Dependency Graph

The 6-tier model below matches `ARCHITECTURE.md` / `AGENTS.md` and is **independently confirmed** by `eslint.config.js`'s `Layers` object and its per-layer `import/no-restricted-imports` rules — the tiering isn't just documentation, a lower tier genuinely cannot `import` a higher one without an ESLint error. This is the single strongest architectural control in the repo.
...
```
Only `@cap/module-auth` and `@cap/module-landing` exist under `packages/modules/`. This is consistent with — and now directly confirms in *live tooling*, not just docs — the phantom-module drift already flagged qualitatively in `technical-recommendations.md` §3. See `technical-debt-report.md` §2.1.
...
### Coupling analysis tooling exists but has not been run
`scripts/analyze-coupling.cjs` is a real, working static-import analyzer that computes afferent/efferent coupling (Ca/Ce) and instability per package and per DDD sub-module, and writes `docs/MODULE_COUPLING_REPORT.md`. **`docs/` does not currently exist in the repo** — this script has apparently never been executed and committed. Recommend running `node scripts/analyze-coupling.cjs` from the repo root (Node + `typescript` available) and committing the output; it will give exact, quantitative Ce/Ca numbers per package/sub-module rather than the qualitative graph in this report. This is a fast, high-value, near-zero-risk win — flagged in the roadmap as the first recommended action.
...
- **Two independently-declared `RouteLayout` type unions** exist (`packages/platform-core/src/assembly/index.tsx` and `packages/platform-core/src/components/LayoutRouteWrapper.tsx`), both `'public' | 'vertical' | 'horizontal' | 'noLayout' | 'admin'`, and a **third, narrower type** `LayoutOverride` in `packages/platform-store/src/store/slices/settingsSlice.ts` (`'public' | 'admin' | 'noLayout' | 'none'` — no `'vertical'`/`'horizontal'`). Three names for overlapping-but-not-identical concepts across three packages is a real coupling smell even though ESLint's layer rule doesn't (and can't) catch it, because a type duplicate isn't an import violation. See §4.1.
...
2. `LayoutRouteWrapper` (both copies — `@cap/layout/src/components/wrappers/LayoutRouteWrapper.tsx` and `@cap/platform-core/src/components/LayoutRouteWrapper.tsx`) only branches on **one** value in its `useEffect`:
   ```tsx
