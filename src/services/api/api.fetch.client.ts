...
import { API_CONFIG } from 'src/services/api/api,config'
...
// Mimic AxiosRequestConfig for compatibility, but tailored for Fetch
...
// Error class to mimic AxiosError
export class HttpError extends Error {
    ...
    constructor(message: string, config: FetchRequestConfig, response?: FetchResponse, code?: string) {
        super(message)
        this.name = 'HttpError'
        this.config = config
...
export class FetchClient {
    ...
    public async request<T = any>(endpoint: string, config: FetchRequestConfig = {}): Promise<FetchResponse<T>> {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 401 && !config._retry && !endpoint.includes('/auth/refresh')) {
                    ...
                    try {
                        ...
                        return this.request<T>(endpoint, { ...config, headers: Object.fromEntries(newHeaders.entries()) })
                ...
                throw new HttpError(responseData?.message || `Request failed with status ${response.status}`, config, result, String(response.status))
            }
            ...
            throw new HttpError(error.message || 'Network Error', config, minimalResponse, 'NETWORK_ERROR')
        }
    }
